  ...
  try {
    ...
    try {
      ...
      return NextResponse.json({
        ...
        responseTime: response.usage?.totalTime || 0
      });
    } catch (requestError) {
      return NextResponse.json({
        success: false,
        ...
        connectionTest: true,
        requestTest: false,
        error: `Request test failed: ${requestError.message}`
      });
    }
  } catch (error) {
    console.error('Error testing LLM provider:', error);
    return NextResponse.json(
      { error: `Failed to test provider: ${error.message}` },
      { status: 500 }
    );
  }
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: `Failed to test providers: ${error.message}` },
      { status: 500 }
    );
  }
}