...
import { Loader2, Lightbulb, Wrench, Send, ClipboardCopy } from 'lucide-react';
...
const ErrorDisplayToast: React.FC<ErrorDisplayToastProps> = ({ error, errorInfo, source }) => {
  ...
  const errorContext = errorInfo?.componentStack || `Source: ${source}\nStack: ${error.stack || 'N/A'}`;
  ...
  const handleExplainError = async () => {
    ...
    try {
      ...
      newToast({ title: "Explanation Failed", description: "Could not get AI explanation.", variant: "destructive" });
    } finally {
  ...
  const handleSuggestFix = async () => {
    ...
    try {
      const input: SuggestCodeFixesInput = {
        error: error.message,
        fileContext: errorContext, // In a real scenario, we'd try to get actual file content
      };
      const result = await suggestCodeFixes(input);
      setSuggestedFix(result);
    } catch (e) {
      console.error("Error getting fix suggestion:", e);
      newToast({ title: "Fix Suggestion Failed", description: "Could not get AI fix.", variant: "destructive" });
    } finally {
  ...
  return (
    <div className="w-full max-w-md space-y-3 text-sm">
      ...
      {errorInfo?.componentStack && (
        <ScrollArea className="h-20 text-xs border rounded bg-destructive/10 p-2 text-destructive-foreground/80">
      ...
      <div className="flex gap-2 flex-wrap">
        <Button size="sm" variant="outline" onClick={handleExplainError} disabled={isLoadingExplanation} className="bg-destructive/20 hover:bg-destructive/30 border-destructive-foreground/50 text-destructive-foreground">
          {isLoadingExplanation ? <Loader2 className="animate-spin" /> : <Lightbulb />} Explain
        </Button>
        <Button size="sm" variant="outline" onClick={handleSuggestFix} disabled={isLoadingFix} className="bg-destructive/20 hover:bg-destructive/30 border-destructive-foreground/50 text-destructive-foreground">
      ...
      {explanation && (
        <Card className="mt-2 bg-background/80 text-foreground">
          <CardHeader className="p-2">
            <CardTitle className="text-xs font-semibold">AI Explanation (Severity: {explanation.severity})</CardTitle>
          </CardHeader>
            ...
            {explanation.trace && explanation.trace.length > 0 && (
                <details className="mt-1">
                    ...
                    <ul className="list-disc list-inside pl-2 text-muted-foreground/80">
                        {explanation.trace.map((step, i) => <li key={i}>{step}</li>)}
                    </ul>
                </details>
            )}
          </CardContent>
      ...
      {suggestedFix && (
        <Card className="mt-2 bg-background/80 text-foreground">
          <CardHeader className="p-2">
            <CardTitle className="text-xs font-semibold">AI Suggested Fix {suggestedFix.confidenceScore ? `(Confidence: ${(suggestedFix.confidenceScore * 100).toFixed(0)}%)` : ''}</CardTitle>
          </CardHeader>
          <CardContent className="p-2 text-xs space-y-2">
            {suggestedFix.explanation && <p className="whitespace-pre-wrap text-muted-foreground mb-1">{suggestedFix.explanation}</p>}
            <Textarea readOnly value={suggestedFix.suggestedCode} rows={4} className="font-mono bg-muted/30 text-xs" />
            {suggestedFix.diff && (
              <div>
                <p className="font-medium text-muted-foreground">Diff:</p>
                <pre className="whitespace-pre-wrap p-1 bg-muted/30 rounded text-xs">{suggestedFix.diff}</pre>
              </div>
            )}
            {suggestedFix.trace && suggestedFix.trace.length > 0 && (
                <details className="mt-1">
                    <summary className="cursor-pointer text-muted-foreground">Show AI Trace</summary>
                    <ul className="list-disc list-inside pl-2 text-muted-foreground/80">
                        {suggestedFix.trace.map((step, i) => <li key={i}>{step}</li>)}
                    </ul>
                </details>
            )}
            <div className="flex gap-2 flex-wrap pt-1">
              <Button size="sm" onClick={handleInjectFix} variant="default"><Send className="mr-1 h-3 w-3" /> Inject to RooCode</Button>
              <Button size="sm" onClick={handleCopyFix} variant="secondary"><ClipboardCopy className="mr-1 h-3 w-3" /> Copy Fix</Button>
              <Button size="sm" onClick={handleRejectFix} variant="outline">Reject Fix</Button>
            </div>
          </CardContent>
        </Card>
      )}
